<template>
  <div ref="mvplay" class="mvplay">
    ...
    <div class="beSimilarMv">
      ...
      <div class="mvlist">
        <ul class="uls">
          <li class="lis" v-for="(item,index) in newMvList" :key="index" @click="playMvs(item.id)">
            <span class="images">
              <span>
...
import { getMvFirst } from '@/network/found'
  export default {
    ...
    mounted(){
      ...
      this.$refs.playmv.style.height = document.body.offsetWidth * 9 / 16 + 'px';
      this.$refs.mvinfo.style.marginTop = 60 + document.body.offsetWidth * 9 / 16 + 'px';
    },
    computed: {
      isMvActiveChange(){
    ...
    watch: {
      ...
      mvPlayID(newVal){
        ...
        getMvUrl(newVal).then((result) => {
          console.log(result);
          this.mvUrl = result.data.url
        }).catch((err) => {
          
        });
        getMvFirst().then((result) => {
          console.log(result);
